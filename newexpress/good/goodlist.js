...
module.exports = {
	...
	goodadd: function (req, res, next) {
		pool.getConnection(function(err, connection) {
			...
			connection.query($sql.goodinsert, [param.id, param.name,param.price,param.img], function(err, result) {
				if(result) {
					result = {
						code: 200,
						msg:'增加成功'
    ...
    goodupdate: function (req, res, next) {
        ...
        pool.getConnection(function(err, connection) {
			...
			// 建立连接，向表中插入值
			// 'INSERT INTO user(id, name, age) VALUES(0,?,?)',
			connection.query($sql.goodupdate, [param.name, param.price,param.img,param.id], function(err, result) {
				if(result) {
					result = {
						code: 200,
						msg:'修改成功'
